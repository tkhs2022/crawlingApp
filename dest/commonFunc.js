...
exports.default = cssFileControl;
exports.cssFileDisable = cssFileDisable;
exports.cssFileAble = cssFileAble;
exports.stringToNumber = stringToNumber;

///////////////////////////////////////////////////////////////// 
// CSSファイルの無効、有効化
function cssFileControl(disableFileName, ableFileName) {
...
function cssFileDisable(disableFileName) {
  for (var i = 0; i < document.styleSheets.length; i++) {
    if (document.styleSheets[i].href != null) {
      var styleSheetPath = document.styleSheets[i].href;
      var fileName = styleSheetPath.substring(styleSheetPath.lastIndexOf('/') + 1, styleSheetPath.length);
...
function cssFileAble(ableFileName) {
  for (var i = 0; i < document.styleSheets.length; i++) {
    if (document.styleSheets[i].href != null) {
      var styleSheetPath = document.styleSheets[i].href;
      var fileName = styleSheetPath.substring(styleSheetPath.lastIndexOf('/') + 1, styleSheetPath.length);
    ...
    if (param == undefined || param == "") {
      ...
      if (isNaN(str)) {
        ...
        if (isNaN(Number(param))) {
          // 1.半角数値の場合
          alert("区分には数値を入力してください!");
          throw "(区分リスト新規登録処理)区分入力エラー"; // str = param;
        } else {
          // 2.半角数字の文字列型の場合
          str = Number(param);
        }
